...
function print(path, options$$1, print) {
  ...
  switch (node.type) {
    ...
    case 'Text':
       ...
       */
      return fill(
        join(line, (node.raw || node.data).split(/[\t\n\f\r ]+/)).parts
      );
    case 'Element':
    ...
    case 'ElseBlock': {
      ...
      if (
        ...
        const def = [
          ...
          '}',
          indent(
            path.map((ifPath) => printChildren(ifPath, print), 'children')[0]
          ),
        ];
        if (ifNode.else) {
          def.push(
...
function embedTag(tag, path, print, textToDoc, node, inline) {
  ...
  const contentAttribute = node.attributes.find(
  ...
  return group$1(
    concat$1([
      ...
      indent$1(
        group$1(
          concat$1(path.map((childPath) => childPath.call(print), 'attributes'))
        )
      ),
      '>',
      indent$1(
        concat$1([hardline$1, nukeLastLine(textToDoc(content, { parser }))])
      ),
      hardline$1,
      '</',
      tag,
